    ...
    <div className="group-not-data-first/block:mt-6">
      ...
      >
        <BaseSelect.Trigger
          className={cn(
            // Base
            'flex h-14 w-full items-center justify-between rounded-(--fun-radius) border-2 border-(--fun-border) px-4 text-base text-(--fun-foreground) transition-all outline-none',
            // Focus
            'focus-visible:border-(--fun-ring) focus-visible:ring-3 focus-visible:ring-(--fun-ring)/50',
            props.static && 'pointer-events-none',
          )}
        >
          <BaseSelect.Value className="data-placeholder:text-(--fun-foreground)/50">
            {(value) => {
        ...
        <BaseSelect.Portal>
          <BaseSelect.Positioner className="z-50 outline-none" sideOffset={8} alignItemWithTrigger={false}>
            <BaseSelect.Popup className="max-h-(--available-height) min-w-(--anchor-width) origin-(--transform-origin) overflow-y-auto rounded-(--fun-radius) bg-(--fun-background) p-1 shadow-md ring-2 ring-(--fun-border) transition-[transform,scale,opacity] slide-in-from-top-2 data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0">
              <BaseSelect.List>
                {options.map((option) => (
                  <BaseSelect.Item
                    key={option.id}
                    value={option.id}
                    className="grid cursor-default scroll-my-1 grid-cols-[1fr_0.75rem] items-center gap-2 rounded-[calc(var(--fun-radius)-3px)] py-3.5 pr-5 pl-3 leading-4 text-(--fun-foreground) transition-colors outline-none select-none data-highlighted:bg-(--fun-muted)"
                  >
